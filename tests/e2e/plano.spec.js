...
for (const ancho of [320, 360]) {
  test(`los cinco formularios y el canvas caben a ${ancho}px`, async ({ page }) => {
    await page.setViewportSize({ width: ancho, height: 740 });
    await page.goto("/");
    for (const metodo of ["newton-raphson", "von-mises", "interpolacion-newton", "punto-medio", "runge-kutta"]) {
